...
test.describe("i18n smoke", () => {
  test("switches hy → ru → en", async ({ page }) => {
    ...
    await expect(page.locator("#calculator h2")).toContainText("Արժեքի հաշվիչ");

    await page.locator("footer").getByRole("link", { name: "Русский" }).click();
    await expect(page).toHaveURL(/\/ru\/?/);
    await expect(page.locator("html")).toHaveAttribute("lang", "ru");
    await expect(page.locator("#calculator h2")).toContainText("Калькулятор");

    await page.locator("footer").getByRole("link", { name: "English" }).click();
    await expect(page).toHaveURL(/\/en\/?/);
    await expect(page.locator("html")).toHaveAttribute("lang", "en");
    await expect(page.locator("#calculator h2")).toContainText("Cost calculator");
  });
});
